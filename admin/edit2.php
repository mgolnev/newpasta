...
if (isset($_GET['order']) || isset($_POST['ohash'])) {
        ...
        $ath9 = mysql_query("SELECT iid as id, name as name, sprice as price, 0 as cat FROM ps_items WHERE vis=1 UNION SELECT ciid as id, ciname as name, ciprice as price, cicat as cat FROM ps_constr WHERE vis=1 ORDER BY cat,name;");
        
        while ($lis = mysql_fetch_array($ath9)) {
                if ($lis['cat']==0) {
...
if (isset($_POST['ohash'])) {
      ...
      //
        ...
        $ohash = $_GET['order'];
    $ath = mysql_query("SELECT * FROM ps_orders LEFT JOIN ps_rajs ON rid = related_raj WHERE ohash = '".addq1($ohash)."';");
   
   if (mysql_numrows($ath)<1) die( "Заказа с таким номером не существует"); else {
        
        $order = mysql_fetch_array($ath);
        $oid = $order['oid'];
        $ath2 = mysql_query("SELECT * FROM ps_it2or LEFT JOIN ps_items ON iid = related_item WHERE related_order = '".$order['oid']."';");
        $items = array();
        while ($item = mysql_fetch_array($ath2)) $items[] = $item;
        
        $ath4 = mysql_query("SELECT * FROM ps_bl2or WHERE related_order = '".$order['oid']."';");
        $builds = array();
        ...
        //rajons
        $ath3 = mysql_query("SELECT * FROM ps_rajs;");
        
        echo "<tr><td>Район:</td><td><select name=raj>";
        while ($chan = mysql_fetch_array($ath3)) {
